...
class Comment(models.Model):
    annoucement = models.ForeignKey(
        Annoucement, verbose_name='Anúncio',
...
def post_save_announcement(instance, created, **kwargs):
    if created:
        ...
        template_name = 'courses/annoucement_mail.html'
...
class Material(models.Model):
    name = models.CharField('Nome', max_length=100)
    embedded = models.TextField('Anexar Vídeo', blank=True)
    file = models.FileField(
        upload_to='lessons/materials',
        blank=True, null=True
    )
    lesson = models.ForeignKey(
        Lesson, related_name='materials', on_delete=models.CASCADE)
